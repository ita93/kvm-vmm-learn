...
static void virtio_pci_space_read(struct virtio_pci_dev *dev, void *data,
                                  uint64_t offset, uint8_t size) {
  if (offset < offsetof(struct virtio_pci_config, dev_cfg)) {
...
static void virtio_pci_space_io(void *owner, void *data, uint8_t is_write,
                                uint64_t offset, uint8_t size) {
...
static void virtio_pci_set_cap(struct virtio_pci_dev *dev, uint8_t next) {
  ...
  // FIXME: the offset for the dev-specifi configuration must be 4-byte aligned.
...
void virtio_pci_set_dev_cfg(struct virtio_pci_dev *dev, void *dev_cfg,
                            uint8_t len) {
  dev->config.dev_cfg = dev_cfg;
  dev->dev_cfg_cap->length = len;
}

void virtio_pci_set_virtq(struct virtio_pci_dev *dev, struct virtq *vq,
                          uint16_t num_queues) {
...
void virtio_pci_set_pci_hdr(struct virtio_pci_dev *dev, uint16_t device_id,
                            uint32_t class, uint8_t irq_line) {
...
void virtio_pci_init(struct virtio_pci_dev *dev, struct pci *pci,
                     struct bus *io_bus, struct bus *mmio_bus) {
  ...
  PCI_HDR_WRITE(dev->pci_dev.hdr, PCI_INTERRUPT_LINE, 1, 8);

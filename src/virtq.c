...
void virtq_complete_request(struct virtq *vq) { vq->ops->complete_request(vq); }

void virtq_notify_used(struct virtq *vq) { vq->ops->notify_used(vq); }

void virtq_enable(struct virtq *vq) { vq->ops->enable_vq(vq); }
